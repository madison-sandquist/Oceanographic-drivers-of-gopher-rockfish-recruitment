...
#### Figure 1 - Recruitment Deviation Timeseries
...
### Oceanographic Timeseries (Figures 2 and 3)

#### Figure 2 Neashore Habitat From 1995 to 2018
...
#### Figure 3 Ocean Basin Indicies Timeseries
...
### March only PSLR
```{r}
df_gopher_mar <- full_data %>% 
  filter(Month %in% c("Mar"))

df_gopher_mar <- df_gopher_mar %>% 
  select(dev, mean_pdo, CUTI, mean_sl,pH_avg, DO_avg, Temperature_avg) %>% 
...
data_complete <- cbind(dev = df_gopher_mar$deviation, X_raw) %>% 
  as.data.frame() %>% 
...
vip_scores <- plsVarSel::VIP(plsr_march, opt.comp = 1)
...
df_all_parameters_yearly_average <- df_all_parameters %>% 
  group_by(Year) %>%
  summarise(across(
    c(DO_avg, pH_avg, Temperature_avg, Vwind_avg,CUTI, mean_pdo, npgo_avg, mean_sl, oni_avg),
    ~ mean(.x, na.rm = TRUE),
    .names = "mean_{.col}"
  )) %>%
  ungroup()
...
# combined 
df_gopher_avg <-left_join(df_all_parameters_yearly_average,goph_recruit, by = "Year") 
...
### Third PSLR averaging from Jan to Aug
...
### Fourth PSLR picking best months for each parameter

```{r}
plsr_df <- full_data %>%
  group_by(Year) %>%
  summarise(
    DO_Jan       = DO_avg[Month == "Jan"][1],
    pH_Mar       = pH_avg[Month == "Mar"][1],
    Temp_Mar     = Temperature_avg[Month == "Mar"][1],
    SeaLevel_Mar = mean_sl[Month == "Mar"][1],
    PDO_Mar      = mean_pdo[Month == "Mar"][1],
    CUTI_Sep     = CUTI[Month == "Sep"][1],
    dev  = dev[1],   # assumes one value per Year; adjust if not
    .groups = "drop"
  )
...
# Define X and Y together and filter complete cases
X_raw <- plsr_df %>% 
  select(PDO_Mar, CUTI_Sep,  SeaLevel_Mar,pH_Mar, DO_Jan, Temp_Mar)

# Combine into one frame with response
data_complete <- cbind(dev = plsr_df$dev, X_raw) %>% 
  as.data.frame() %>% 
  na.omit()  # remove rows with any NA
...
# 1-component model is optimal, so we use opt.comp = 1
vip_scores <- VIP(plsr_model, opt.comp = 2)
...
# Use same predictors from the original full dataset for correlation
X_original <- plsr_model%>% select(all_of(predictor_names))
Y_original <- plsr_model$dev
...
fig_a_best <- ggplot(loadings_df, aes(x = Component1, y = Component2)) +
  # Unit circle
  geom_path(data = data.frame(
    x = cos(seq(0, 2 * pi, length.out = 100)),
    y = sin(seq(0, 2 * pi, length.out = 100))
  ...
  xlim(-1.1, 1.1) + ylim(-1.1, 1.1) +
  labs(x = "Component 1", y = "Component 2", title = "Strongest Correlation Months") +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),
    axis.line = element_line(color = "black"),
...
fig_b_best <- ggplot(data.frame(Component1 = component1_scores, Response = response_var),
                aes(x = Component1, y = Response)) +
  geom_point(size = 2.5, alpha = 0.7) +
  geom_smooth(method = "lm", color = "steelblue", fill = "black", alpha = 0.3) +
  annotate("text", 
...
fig_c_best <- ggplot(vip_df, aes(x = reorder(Variable, VIP), y = VIP)) +
  geom_col(fill = "steelblue") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "indianred4", size = 1) +
  geom_hline(yintercept = 0.8, linetype = "dashed", color = "indianred3", size = 1) +
  coord_flip() +
...
# ========== COMBINE ALL THREE FIGURES ==========
combined_plot <- fig_a_best + fig_b_best + fig_c_best + 
  plot_layout(ncol = 3)
combined_plot
...
#### Figure 5 - Combination of PLSR

```{r}
combined_plot <- fig_a_march + fig_b_march + fig_c_march + fig_a_year + fig_b_year + fig_c_year + fig_a_repro + fig_b_repro + fig_c_repro +
  plot_layout(ncol = 3) 
combined_plot
ggsave('Figures/fig3.png', width = 15, height = 10, dpi = 300)
```

```{r}
combined_plot <- fig_a_march + fig_b_march + fig_c_march + fig_a_repro + fig_b_repro + fig_c_repro + fig_a_best + fig_b_best + fig_c_best +
  plot_layout(ncol = 3) 
combined_plot
ggsave('Figures/fig5.png', width = 15, height = 10, dpi = 300)
```

## Best historical predictor
...
## Join everything
df_env_joined <- left_join(ROMS_outputs, goph_recruit, by = "Year") %>%
  select(Year, Month, DO_avg, pH_avg, Temperature_avg, dev)
...
df_windows <- df_long %>%
  ...
  mutate(
    ...
    value_6mo = slide_dbl(value, mean, .before = 5, .complete = TRUE, na.rm = FALSE)
  ) %>%
  ungroup()
...
ggsave('Figures/fig6.png', width = 7, height = 4, dpi = 300)
```
...
### Future Oceanographic Characteristcs 
```{r}
...
```
### ANOVA for future conditions 
```{r}
# DO
anova_do <- aov(Value ~ Period + ESM,
                data = dplyr::filter(future_long, Variable == "DO"))
summary(anova_do)

# pH
anova_ph <- aov(Value ~ Period + ESM,
                data = dplyr::filter(future_long, Variable == "pH"))
summary(anova_ph)

# Temperature
anova_temp <- aov(Value ~ Period + ESM,
                  data = dplyr::filter(future_long, Variable == "Temperature"))
summary(anova_temp)
```
...
for (name in names(plots_timeseries)) {
  ggsave(
    filename = file.path("figures", paste0(name, ".png")),
    plot = plots_timeseries[[name]],
    width = 8,
    height = 4,
    dpi = 300
  )
